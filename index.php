<!DOCTYPE html>
<html lang="">

<head>
	<meta charset="utf-8">
	<title>Example Title</title>
	<meta name="author" content="Your Name">
	<meta name="description" content="Example description">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" href="">
	<link rel="icon" type="image/x-icon" href=""/>
</head>

<body>
	<header></header>
	<main>
        
        <div id="getInputText">
            <textarea id="inputTextSsl" style="height: 400px; width: 400px; ">
                -----BEGIN PRIVATE KEY----------END PRIVATE KEY-----
            </textarea>
        </div>
        <div>
            <button id="formatSslFormatter" onclick="formatSslFormatter();">Format SSL Certificate</button>
        </div>
        <div>
            <button id="buttonCopy" onclick="copyClipText();">Copy To Clipboard</button>
        </div>
        <div>
            <div><button onclick="certificateDownload('crt');">Download .crt File</button></div>
            <div><button onclick="certificateDownload('pem');">Download .pem File</button></div>
            <div><button onclick="certificateDownload('cer');">Download .cer File</button></div>
        </div>
        <div>
            <textarea id="formatSSLCertificate" style="height: 500px; width: 500px; "></textarea>
        </div>
        
        
        
        
        .crt,.pem,.key
        
        Base64 (ASCII)
PEM
.pem
.crt
.ca-bundle
PKCS#7
.p7b
.p7s
Binary
DER
.der
.cer
PKCS#12
.pfx
      
*.pem, *.crt, *.ca-bundle, *.cer, *.p7b, *.p7s        
        
        
        
 <!--       <section id="topBoxContainer">
            <div class="container">
                <div class="row">
                    <div class="col-md-6">
                        <div class="sslbox-area">
                            <textarea id="inputssl"></textarea>
                        </div>
                    </div>
                     <div class="col-md-6">
                        <div class="sslbox-area">
                            <textarea id="outputssl"></textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="formatButtonSsl">
                            <button id="formatSslButton">Format</button>
                        </div>
                    </div>
                </div>
            </div>
        </section>-->
    </main>
	<footer></footer>
	<script type="text/javascript" src="https://code.jquery.com/jquery-3.6.1.min.js"></script>
    <script type="text/javascript">
         function formatSslFormatter(){
                    var getInputData = jQuery('#inputTextSsl').text().trim().replaceAll('-----BEGIN PRIVATE KEY-----','').replaceAll("-----END PRIVATE KEY-----",'');

                 //   console.log(getInputData);
                    var newStringArray = []; 
                    newStringArray.push(getInputData.split(""));
                 //  console.log(newStringArray[0]);
                    var outputText = '';
             
                    outputText += '-----BEGIN CERTIFICATE-----';
                    outputText += '\n';
                    outputText += newStringArray[0][0];
                     var k = 0; 
                    for(var i = 2; i < newStringArray[0].length; i++){
                        outputText += newStringArray[0][i]; 
                        if(i%64 == 0){
                            outputText += '\n';
                           } 
                    }
                    outputText += '\n';
                    outputText += '-----END CERTIFICATE-----';
             
                  //  console.log(outputText);
                    jQuery('#formatSSLCertificate').text(outputText);
            }
        
        function copyClipText(){
             var copyText = document.getElementById("formatSSLCertificate");
          copyText.select();
          document.execCommand("copy");
        }
        
        function certificateDownload(fileType){
            
//            alert(fileFt);

            var getCertificate = jQuery('#formatSSLCertificate').text(); 
            
            var data = {'fileType':fileType, 'certificateText':getCertificate}; 
            
            $.ajax({
                url:'create.php',
                type: 'POST',
                data: data,
                success: function(response){
                    console.log(response);
                }
            });
            
            
        }
            
            
    </script>
</body>

</html>



